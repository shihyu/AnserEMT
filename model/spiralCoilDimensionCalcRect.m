function [ x_points_out,y_points_out,z_points_out ] = spiralCoilDimensionCalcRect( N,length_1,length_2,width,spacing,thickness,angle )
%SPIRALCOILDIMENSIONCALCRECT Summary of this function goes here
...
%Define dimensions
len_1 = length_1; %define side length of one side of coil
len_2 = length_2;
...
for seg_move = 4:2:(2* N )
    line_segs(seg_move) = len_2 - i*ll_s;
    line_segs(seg_move + 1) = len_1 - i*ll_s;
    i = 1+i;
end
...
for seg_move = 2* N :2:(4* N )-1
    line_segs(seg_move) = len_2 - i_smaller * ll_s;
    line_segs(seg_move + 1) = len_1 - i_smaller * ll_s;
    i = 1+i;
    i_smaller = i_smaller-1;
end


line_segs(4*N - 2) = len_2;
line_segs(4*N - 1) = len_1;
line_segs(4*N) = len_2;
...
if angle == pi/2
    x_points_out = x_points_out+len_2/2;
    y_points_out = y_points_out-len_1/2;
    
else
    
    y_points_out = y_points_out-length_1/(sqrt(2));
end
